namespace Slurper
{
    ...
    internal class Helpers
    {
            ...
            for (int i = 0; i < knownCompanyBrandRelationships.Count - 1; i++)
            {
                ...
                nonRelationships.Add(
                                     new CompanyAndBrands
                                         {
                                                 CompanyName = knownCompanyBrandRelationships[i].CompanyName,
            ...
            foreach (var page in pages)
            {
                ...
                foreach (var initialcandidate in initialCandidateSegments)
                {
                    ...
                    foreach (var company in companiesMentionedOnPage)
                    {
                        ...
                        var previousContentOuterHtml = previousContent == null ? "" : previousContent.OuterHtml;

                        var previousContentWithoutCompany = Regex.Replace(previousContentOuterHtml, company, "");

                        var previousContentInnerText = previousContent == null ? "" : previousContent.InnerText;

                        var previousContentInnerTextWithoutEntity = Regex.Replace(previousContentInnerText, company, "");

                        var domainOrTitleContainsPotentialOwner = DomainOrTitleContainsPotentialOwner(
                                                                                                      page,
                                                                                                      company,
                                                                                                      title);

                        var candidateHtmlContainsPotentialOwner = CandidateContainsPotentialOwner(
                                                                                                  initialcandidate,
                                                                                                  company);

                        var previousContentContainsPotentialOwner =
                                PreviousContentContainsPotentialOwner(previousContentInnerText, company);

                        var nearestHeadingIndicatesNoRelation =
                                nearestHeadingAbove.ToLowerInvariant().Contains("external links")
                                || nearestHeadingAbove.ToLowerInvariant().Contains("references")
                                || nearestHeadingAbove.ToLowerInvariant().Contains("by appointment to")
                                || nearestHeadingAbove.ToLowerInvariant().Contains("further reading");

                        var previousContentIndicatesNoRelation =
                                previousContentInnerText.ToLowerInvariant().Contains("external links")
                                || previousContentInnerText.ToLowerInvariant().Contains("references")
                                || previousContentInnerText.ToLowerInvariant().Contains("by appointment to")
                                || previousContentInnerText.ToLowerInvariant().Contains("further reading");
                        ...
                        if (!nearestHeadingIndicatesNoRelation
                            && (!previousContentIndicatesNoRelation
                                && (numberOfLanguageNames < 5
                                    && (domainOrTitleContainsPotentialOwner || candidateHtmlContainsPotentialOwner
                                        || previousContentContainsPotentialOwner))))
                        {
                            var captions = initialcandidate.Node.SelectNodes("caption");

                            var captionsContainOwner = captions != null
                                                       && captions.Any(c => c.InnerText.Contains(company));

                            var brandsPresentInInitialCandidate = new List<string>();

                            foreach (var brand in brands)
                            {
                                if (company.ToLowerInvariant() != brand.ToLowerInvariant()
                                    && ContainsNounPhrase(brand, tagger))
                                {
                            ...
                            if (brandsPresentInInitialCandidate.Count > 0)
                            {
                                ...
                                {
                                    wordsInPreviousContent =
                                            previousContent.InnerText.Split(
                                                                            new char[]
                                                                                {
                                                                                        '.',
                                                                                        '?',
                                ...
                                foreach (var brand in brandsPresentInInitialCandidate)
                                {
                                    ...
                                    innerSegments.AddRange(
                                                           allInnerSegments.Where(
                                                                                  ins =>
                                                                                  ins.InnerText.ToLowerInvariant()
                                        ...
                                        var item = new ListOrTableItem
                                                       {
                                                               ...
                                                               ItemHtmlWithoutBrand =
                                                                       safey.Sanitize(htmlWithoutBrand),
                                                               ItemInnerText = trimmedText,
                                                               ItemWordCount =
                                                                       wordsInInnerSegment.Count(),
                                                               WordsInItem = wordsInInnerSegment,
                                                               KnownBrand = brand,
                                                               ContainsBrandOnly =
                                                                       innerSegment.InnerText
                                                                                   .ToLowerInvariant()
                                                                                   .Equals(
                                                                                           brand
                                                                                                   .ToLowerInvariant
                                            ...
                                            var candidate = new Candidate
                                                                {
                                                                        ...
                                                                        PreviousContentWordCount =
                                                                                wordsInPreviousContent.Count(),
                                                                        WordsInPreviousContent =
                                                                                wordsInPreviousContent,
                                                                        CandidateHtml =
                                                                                listOrTableItemContainingBrand
                                                                                .ItemHtml,
                                                                        CandidateHtmlWithoutCandidateEntities
                                                                                =
                                                                                listOrTableItemContainingBrand
                                                                                .ItemHtmlWithoutBrand,
                                                                        CandidateHtmlWordCount =
                                                                                listOrTableItemContainingBrand
                                                                                .ItemWordCount,
                                                                        NumberOfLinksInCandidateHtml =
                                                                                numberOfLinksInCandidateHtml,
                                                                        WordsInCandidateHtml =
                                                                                listOrTableItemContainingBrand
                                                                                .WordsInItem,
                                                                        NearestHeadingAbove =
                                                                                nearestHeadingAbove,
                                                                        ...
                                                                        PreviousContentContainsPotentialOwner
                                                                                =
                                                                                previousContentContainsPotentialOwner,
                                                                        CandidateHtmlContainsPotentialOwner =
                                                                                candidateHtmlContainsPotentialOwner,
                                                                        Uri = page,
                                                                        PageTitle = title,
                                                                        ContainsMultipleBrands =
                                                                                brandsPresentInInitialCandidate
                                                                                        .Count > 1,
                                                                        ItemsContainBrandOnly =
                                                                                listOrTableItemContainingBrand
                                                                                .ContainsBrandOnly,
                                                                        CaptionsContainOwner =
                                                                                captionsContainOwner,
                                                                        NumberOfItemsWithLanguages =
                                                                                numberOfLanguageNames,
                                                                        NumberOfLocationNames =
                                                                                numberOfLocations,
                                                                        NumberOfOrganisationNames =
                                ...
                                {
                                    var wordsInCandidateHtml =
                                            initialcandidate.Node.InnerText.Split(
                                                                                  new char[]
                                                                                      {
                                                                                              '.',
                                                                                              '?',
                                    ...
                                    bool noOtherTextExceptBrands = !itemsWithBrandsForWholeCandidate.IsNullOrEmpty()
                                                                   && itemsWithBrandsForWholeCandidate.TrueForAll(
                                                                                                                  iwb =>
                                                                                                                  iwb
                                                                                                                          .ContainsBrandOnly);

                                    bool someItemsHaveNoOtherTextExceptBrand =
                                            !itemsWithBrandsForWholeCandidate.IsNullOrEmpty()
                                            && itemsWithBrandsForWholeCandidate.Any(iwb => iwb.ContainsBrandOnly);
                                        ...
                                        if (someItemsHaveNoOtherTextExceptBrand)
                                        {
                                            foreach (var segment in allInnerSegments)
                                            {
                                                ...
                                                if (!candidate.KnownBrands.Contains(cleanedSegment)
                                                    && ContainsNounPhrase(cleanedSegment, tagger))
                                                {
                ...
                int positivesCount = 100)
        {
            ...
            foreach (var page in pages)
            {
                if (!positiveCandidates && candidates.Count >= positivesCount)
                {
                    break;
                }
                else
                {
                    ...
                    foreach (var initialcandidate in initialCandidateSegments)
                    {
                        ...
                        foreach (var relation in relationsWhereOwnerMentionedOnPage)
                        {
                            ...
                            var previousContentOuterHtml = previousContent == null ? "" : previousContent.InnerText;
                            ...
                            var domainOrTitleContainsOwner = DomainOrTitleContainsPotentialOwner(
                                                                                                 page,
                                                                                                 relation.CompanyName,
                                                                                                 title);
                            ...
                            if (domainOrTitleContainsOwner || candidateHtmlContainsPotentialOwner
                                || previousContentContainsPotentialOwner)
                            {
                                ...
                                var captionsContainOwner = captions != null
                                                           && captions.Any(
                                                                           c =>
                                                                           c.InnerText.Contains(relation.CompanyName));
                                ...
                                foreach (var brand in relation.BrandNames)
                                {
                                    ...
                                    if (brandOnItsOwn.IsMatch(initialcandidate.Node.InnerText.ToLowerInvariant())
                                        && relation.CompanyName.ToLowerInvariant() != brand.ToLowerInvariant())
                                    {
        ...
        internal static List<string> GetFullNounPhrasesFromString(string text, MaxentTagger tagger)
        {
            ...
            foreach (var line in lines)
            {
                ...
                {
                    if (TaggedTextContainsNounPhrase(tokens[i]) && (i + 1) != tokens.Count
                        && TaggedTextContainsNounPhrase(tokens[i + 1]))
                    {
        ...
        {
            return
                    Directory.GetFiles(path, "*.htm*", SearchOption.AllDirectories)
                             .Where(f => !f.Contains("Image~") && !f.Contains("Talk~"));
        }

        internal static List<string> GetTestBrands(
                ...
                CRFClassifier classifier)
        {
            ...
            var entityPattern = new Regex(@"ORGANIZATION\s+|LOCATION\s+|PERSON\s+");

            foreach (var brand in uniqueBrands)
            {
                var trimmedBrand = brand.Trim();

                var nerTaggedInnerText = classifier.classifyToString(brand);

                if (!entityPattern.IsMatch(nerTaggedInnerText))
                {
            ...
            var troublesome2LetterBrands = new List<String>
                                               {
                                                       "ee",
                                                       "et",
                                                       "ry",
            ...
            var brandStopList = new List<String>
                                    {
                                            "bury",
                                            "ting",
                                            "may",
            ...
            foreach (var candidate in candidates)
            {
                if (candidate.KnownBrands != null)
                {
                    file.WriteLine(
                                   ...
                                   + Environment.NewLine + Environment.NewLine + "Domain/ Title contain owner: "
                                   + candidate.DomainOrPageTitleContainsOwner + Environment.NewLine
                                   + Environment.NewLine + "Known company: " + candidate.KnownCompanyName
                                   + Environment.NewLine + "Known brand: " + candidate.KnownBrand + Environment.NewLine
                                   + Environment.NewLine + "Known brands: " + String.Join(", ", candidate.KnownBrands)
                                   + Environment.NewLine + Environment.NewLine + "Previous heading: "
                                   + String.Join(", ", candidate.NearestHeadingAbove) + Environment.NewLine
                                   + Environment.NewLine + "Location names: "
                                   + String.Join(", ", candidate.NumberOfLocationNames) + Environment.NewLine
                                   + Environment.NewLine + "Person names: "
                                   + String.Join(", ", candidate.NumberOfPersonNames) + Environment.NewLine
                                   + Environment.NewLine + "Organisation names: "
                                   + String.Join(", ", candidate.NumberOfOrganisationNames) + Environment.NewLine
                                   + Environment.NewLine + "Contains multiple language names: "
                                   + String.Join(", ", candidate.NumberOfItemsWithLanguages) + Environment.NewLine
                                   + Environment.NewLine + "Candidate HTML word count: "
                                   + String.Join(", ", candidate.CandidateHtmlWordCount) + Environment.NewLine
                                   + Environment.NewLine + "Number of links in HTML: "
                                   + String.Join(", ", candidate.NumberOfLinksInCandidateHtml) + Environment.NewLine
                                   + Environment.NewLine + "Previous content word count: "
                                   + String.Join(", ", candidate.PreviousContentWordCount) + Environment.NewLine
                                   + Environment.NewLine + "Previous content contains potential owner: "
                                   + String.Join(", ", candidate.PreviousContentContainsPotentialOwner)
                                   + Environment.NewLine + Environment.NewLine
                                   + "Candidate HTML contains potential owner: "
                                   + String.Join(", ", candidate.CandidateHtmlContainsPotentialOwner)
                                   + Environment.NewLine + Environment.NewLine + "Captions contain owner: "
                                   + String.Join(", ", candidate.CaptionsContainOwner) + Environment.NewLine
                                   + Environment.NewLine + "Candidate HTML contains potential owner: "
                                   + String.Join(", ", candidate.CandidateHtmlContainsPotentialOwner)
                                   + Environment.NewLine + Environment.NewLine + "Some items contain brand only: "
                                   + String.Join(", ", candidate.ItemsContainBrandOnly) + Environment.NewLine
                                   + Environment.NewLine + "Contains Multiple brands?"
                                   + candidate.ContainsMultipleBrands + Environment.NewLine
                ...
                Console.WriteLine(
                                  candidate.PageTitle + Environment.NewLine + ' ' + candidate.KnownCompanyName
                                  + Environment.NewLine + ' ' + candidate.CandidateHtmlWithoutCandidateEntities
                                  + Environment.NewLine);
            }
        ...
        internal static void OutputJsonResults(List<Candidate> candidates)
        {
            ...
            var uniqueGroup = classifiedRelations.GroupBy(cr => new { cr.Company, cr.Brand }).Where(g => g.Count() <= 1);
        ...
        {
            return (text.Contains("_NN") || text.Contains("_NNS") || text.Contains("_NNP") || text.Contains("_NNS"));
        }
        ...
        {
            bool candidateHtmlContainsPotentialOwner =
                    initialcandidate.Node.InnerText.ToLowerInvariant().Contains(company.ToLowerInvariant() + " ");
        ...
        {
            bool domainOrTitleContainsPotentialOwner = page.ToLowerInvariant()
        ...
        private static List<string> GetCompaniesPresentOnPage(List<string> companies, HtmlNode root, string page)
        {
            ...
            foreach (var company in companies)
            {
                var companyOnItsOwn = new Regex(@"\b" + company.ToLowerInvariant() + @"\b");
                if (companyOnItsOwn.IsMatch(root.OuterHtml.ToLowerInvariant())
                    || companyOnItsOwn.IsMatch(page.ToLowerInvariant()))
                {
                ...
                string page)
        {
            ...
            {
                var companyOnItsOwn = new Regex(@"\b" + relation.CompanyName.ToLowerInvariant() + @"\b");
                if (relation.CompanyName != null
                    && (companyOnItsOwn.IsMatch(root.OuterHtml.ToLowerInvariant())
                        || companyOnItsOwn.IsMatch(page.ToLowerInvariant())))
                {
        ...
        private static HtmlNode GetPreviousRelevantNode(InitialCandidate initialcandidate)
        {
            ...
            if (previousSibling != null && !previousSibling.InnerText.IsNullOrEmpty()
        ...
        private static List<InitialCandidate> InitialCandidateSegments(HtmlNode root)
        {
            var initialCandidateSegments =
                    root.Descendants("table")
                        ...
                        .Where(
                               table =>
                               !table.OuterHtml.Contains("<ul>")
                               && !table.ChildNodes.Any(cn => cn.InnerText.Length > 300)
                               && !(table.Descendants("tr").ToList().Count > 100))
            ...
            initialCandidateSegments.AddRange(
                                              root.Descendants("ul")
                                                  ...
                                                  .Where(
                                                         list =>
                                                         !list.InnerHtml.Contains("<ul>")
                                                         && !list.ChildNodes.Any(cn => cn.InnerText.Length > 300)
                                                         && !(list.Descendants("li").ToList().Count > 100))
        ...
        {
            bool previousContentContainsPotentialOwner =
                    previousContentInnerText.ToLowerInvariant().Contains(company.ToLowerInvariant() + " ");
